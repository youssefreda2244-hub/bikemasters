(() => {
  ...
  function renderCatalog() {
    ...
    ui.grid.innerHTML = filteredProducts.map(p => {
      ...
      return `<article class="ticket${inStock ? '' : ' ticket--sold-out'}">
        ...
        ${inStock && p.badge ? `<span class="ticket-badge">${esc(p.badge)}</span>` : ''}${extraPhotoCount ? `<span class="ticket-photos" title="${extraPhotoCount} more photos">▣ +${extraPhotoCount}</span>` : ''}</div>
        <div class="ticket-body"><h3>${esc(p.name)}</h3>${inStock ? `<p class="ticket-card-price">${money(p.price)}</p>` : ''}<div class="ticket-sub">${esc(p.subtitle || (inStock ? `${p.quantity} IN STOCK` : 'SOLD OUT'))}</div><div class="ticket-perf"></div>
  ...
  async function renderAdminOrders() {
    ...
    if (error) { box.innerHTML = `<p class="admin-note">Orders could not be loaded: ${esc(error.message || 'unknown database error')}<br>Run orders-setup.sql in the Supabase SQL Editor.</p>`; return; }
    box.innerHTML = data?.length ? data.map(order => `<article class="admin-order"><div><strong>${esc(order.customer_name)}</strong><span>${new Date(order.created_at).toLocaleString()} · ${esc(order.phone)} · ${esc(order.city)}</span><span class="order-address"><b>Address:</b> ${esc(order.address)}</span><span>${(order.order_items || []).map(item => `${esc(item.product_name)} × ${item.quantity}`).join(' · ')}</span></div><div><b>${money(order.total)}</b><select data-order-status="${order.id}"><option value="new" ${order.status === 'new' ? 'selected' : ''}>New</option><option value="confirmed" ${order.status === 'confirmed' ? 'selected' : ''}>Confirmed</option><option value="preparing" ${order.status === 'preparing' ? 'selected' : ''}>Preparing</option><option value="shipped" ${order.status === 'shipped' ? 'selected' : ''}>Shipped</option><option value="delivered" ${order.status === 'delivered' ? 'selected' : ''}>Delivered</option><option value="cancelled" ${order.status === 'cancelled' ? 'selected' : ''}>Cancelled</option></select></div></article>`).join('') : '<p class="admin-note">No orders yet.</p>';
