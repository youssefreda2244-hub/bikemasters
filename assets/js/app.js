  ...
  function renderCatalog() {
    ...
    ui.grid.innerHTML = filteredProducts.map(p => {
      ...
      return `<article class="ticket${inStock ? '' : ' ticket--sold-out'}">
        ...
        ${extraPhotoCount ? `<span class="ticket-photos" title="${extraPhotoCount} more photos">▣ +${extraPhotoCount}</span>` : ''}</div>
        <div class="ticket-body"><h3>${esc(p.name)}</h3>${inStock ? `<p class="ticket-card-price">${money(p.price)}</p>${p.badge ? `<span class="ticket-badge">${esc(p.badge)}</span>` : ''}` : ''}<div class="ticket-sub">${esc(p.subtitle || (inStock ? `${p.quantity} IN STOCK` : 'SOLD OUT'))}</div><div class="ticket-perf"></div>
