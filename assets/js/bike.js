(() => {
  ...
  db.from('products').select('*').eq('id', id).maybeSingle().then(({ data: p, error }) => {
    ...
    target.innerHTML = `<article class="bike-detail"><div class="bike-gallery"><img id="main-bike-photo" src="${esc(photos[0] || 'assets/images/image-01.jpg')}" alt="${esc(p.name)}"><div class="bike-thumbnails">${photos.map((url, i) => `<button class="bike-thumb${i === 0 ? ' is-active' : ''}" type="button" data-photo="${esc(url)}"><img src="${esc(url)}" alt="${esc(p.name)} photo ${i + 1}"></button>`).join('')}</div></div><div class="bike-copy"><span class="eyebrow">${available ? `Available now · ${p.quantity} in stock` : 'Sold out'}</span><h1>${esc(p.name)}</h1><p class="bike-price">${money(p.price)}</p><p class="bike-subtitle">${esc(p.subtitle || '')}</p><ul class="spec-list">${specs || '<li><span>Details</span><span>Contact shop</span></li>'}</ul>${available ? `<div class="product-quantity-control"><button type="button" data-qty-minus aria-label="Decrease quantity">−</button><input id="product-quantity" value="1" max="${Math.max(1, Number(p.quantity) || 1)}" inputmode="numeric" aria-label="Quantity"><button type="button" data-qty-plus aria-label="Increase quantity">+</button></div><div class="purchase-actions"><button class="btn btn-primary" type="button" data-cart-add="${esc(p.id)}">Add to cart</button><button class="btn btn-ghost" type="button" data-cart-buy="${esc(p.id)}">Buy now</button></div><a class="btn btn-ghost bike-contact" href="https://wa.me/${config.whatsappNumber}?text=${message}" target="_blank" rel="noopener">Ask about this bike</a>` : '<p class="catalog-message">Currently unavailable.</p>'}</div></article>`;
    if (window.BikeCart) window.BikeCart.setProducts([p]);
    let activePhoto = 0;
